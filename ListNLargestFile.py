...
def checkandappendfile ( filename , Nlargest ):
    size =  os.path.getsize(filename) 
   # tupleitem = (  size , filename) 
    tupleitem = (    filename ,  size ) 
    
    
    if ( len(NLargestFileList)< Nlargest ):
        NLargestFileList.append( tupleitem)
        NLargestFileList.sort( reverse=True, key= lambda x:(x [1] ) )
    else:
        if( NLargestFileList[ Nlargest-1 ][1] < size ):
           NLargestFileList.pop()
           NLargestFileList.append(tupleitem) 
           NLargestFileList.sort( reverse=True, key= lambda x:(x [1] ) )
