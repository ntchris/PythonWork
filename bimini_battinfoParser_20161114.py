...
class BattInfoParserClass:
   ...
   @staticmethod
   def createErrorLinesFile(errorfilename, errorLineList):
      errorfile=open(errorfilename, "w")
      errorfile.write("orig line #     corrupted line\n")
      for line in errorLineList:
         errorfile.write(str(line[0]) + "   " + line[1]+"\n" )

      errorfile.close()
      print ("Corrupted lines written in file:\n    "+errorfilename+".  You can manually analyze those corrupted lines")
...
def main():
   ...
   i=0
   # use this to output help message
   isFirstErrorLine = True;
   for origline in origlogfile:
     ...
     try:     
        ...
        if( isFirstErrorLine):
           isFirstErrorLine = False
           print ("Error two alarm data in one line! # ", end=" ")
        else:
           print(str(i)+ " ", end=" ")
        tuple=i, origline
        errorLineList.append(tuple)
   ...
   # print corrupted lines so user can manually process
   
   printLine()
   
   #print("BatteryInfo parsed: "+str(parsedCount ))
   
   print("\n\n "+ str(parsedCount )+ " battery Info log parsed, new CSV file generated: " + output_battInfoCSV_filename+"\n")
   print("If using Office365, new an empty doc, choose Data->From Text -> choose semicolon as seperator")
   printLine()
   BattInfoParserClass.createErrorLinesFile(origBattInfoLogFile+"_errorLines.txt", errorLineList)
